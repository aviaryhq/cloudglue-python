...
class CollectionsApi:
    ...
    def get_entities(
        ...
        include_thumbnails: Annotated[Optional[StrictBool], Field(description="When true, include a file-level thumbnail_url in the response and a per-segment thumbnail_url on each segment entity")] = None,
        include_chapters: Annotated[Optional[StrictBool], Field(description="Include narrative chapters in the response (when segmentation strategy is 'narrative')")] = None,
        include_shots: Annotated[Optional[StrictBool], Field(description="Include shot boundaries in the response (when segmentation strategy is 'shot-detector')")] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        :type offset: int
        :param include_thumbnails: When true, include a file-level thumbnail_url in the response and a per-segment thumbnail_url on each segment entity
        :type include_thumbnails: bool
        :param include_chapters: Include narrative chapters in the response (when segmentation strategy is 'narrative')
        :type include_chapters: bool
        :param include_shots: Include shot boundaries in the response (when segmentation strategy is 'shot-detector')
        :type include_shots: bool
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
    ...
    def _get_entities_serialize(
        ...
        if include_thumbnails is not None:
            ...
            _query_params.append(('include_thumbnails', include_thumbnails))
            
        if include_chapters is not None:
            
            _query_params.append(('include_chapters', include_chapters))
            
        if include_shots is not None:
            
            _query_params.append(('include_shots', include_shots))
    ...
    def get_media_descriptions(
        ...
        include_thumbnails: Annotated[Optional[StrictBool], Field(description="When true, include a file-level thumbnail_url on the response and per-segment thumbnail_url on each segment_summary entry")] = None,
        include_word_timestamps: Annotated[Optional[StrictBool], Field(description="When true, include a words array on each speech entry with word-level start_time and end_time. Not available for YouTube sources. Only applies when response_format=json.")] = None,
        include_chapters: Annotated[Optional[StrictBool], Field(description="Include narrative chapters in the response (when segmentation strategy is 'narrative')")] = None,
        include_shots: Annotated[Optional[StrictBool], Field(description="Include shot boundaries in the response (when segmentation strategy is 'shot-detector')")] = None,
        _request_timeout: Union[
            None,
        ...
        :param include_thumbnails: When true, include a file-level thumbnail_url on the response and per-segment thumbnail_url on each segment_summary entry
        :type include_thumbnails: bool
        :param include_word_timestamps: When true, include a words array on each speech entry with word-level start_time and end_time. Not available for YouTube sources. Only applies when response_format=json.
        :type include_word_timestamps: bool
        :param include_chapters: Include narrative chapters in the response (when segmentation strategy is 'narrative')
        :type include_chapters: bool
        :param include_shots: Include shot boundaries in the response (when segmentation strategy is 'shot-detector')
        :type include_shots: bool
    ...
    def _get_media_descriptions_serialize(
        ...
        if include_word_timestamps is not None:
            
            _query_params.append(('include_word_timestamps', include_word_timestamps))
    ...
    def list_videos(
        ...
        order: Annotated[Optional[StrictStr], Field(description="Order the files by a specific field")] = None,
        sort: Annotated[Optional[StrictStr], Field(description="Sort the files in ascending or descending order")] = None,
        filter: Annotated[Optional[StrictStr], Field(description="JSON string containing filter criteria to constrain file search results. This is the JSON string version of the SearchFilter object used in the search API.  **Supported Filter Types:**  • **metadata** - Filter by file metadata using JSON path expressions (e.g., 'metadata.speaker', 'metadata.category.subcategory') • **video_info** - Filter by video information properties. Supports optional `scope` ('file' or 'segment', default 'file')   - `duration_seconds` - Video/segment duration in seconds (supports file and segment scope)   - `has_audio` - Whether the video has audio (true/false, file scope only)  • **file** - Filter by file properties   - `filename` - File name (string)   - `uri` - File URI (string)   - `id` - File ID (UUID string)   - `created_at` - Creation timestamp (ISO 8601 string)   - `bytes` - File size in bytes (number)  **Supported Operators:** • **Equal** - Exact match (requires valueText) • **NotEqual** - Not equal to value (requires valueText) • **LessThan** - Less than value (requires valueText) • **GreaterThan** - Greater than value (requires valueText) • **Like** - Case-insensitive pattern matching with wildcards (requires valueText) • **In** - Value is in array (requires valueTextArray) • **ContainsAny** - Array contains any of the values (requires valueTextArray) • **ContainsAll** - Array contains all of the values (requires valueTextArray)  **Examples:**  **Metadata filtering:** ```json {\"metadata\":[{\"path\":\"speaker\",\"operator\":\"Like\",\"valueText\":\"YO%\"}]} ```  **Video info filtering (file-level):** ```json {\"video_info\":[{\"path\":\"duration_seconds\",\"operator\":\"GreaterThan\",\"valueText\":\"60\"}]} ```  **Video info filtering (segment-level):** ```json {\"video_info\":[{\"path\":\"duration_seconds\",\"operator\":\"LessThan\",\"valueText\":\"30\",\"scope\":\"segment\"}]} ```  **File property filtering:** ```json {\"file\":[{\"path\":\"filename\",\"operator\":\"Like\",\"valueText\":\"%.mp4\"}]} ``` ```json {\"file\":[{\"path\":\"bytes\",\"operator\":\"GreaterThan\",\"valueText\":\"1048576\"}]} ``` ```json {\"file\":[{\"path\":\"created_at\",\"operator\":\"GreaterThan\",\"valueText\":\"2024-01-01T00:00:00Z\"}]} ```  **Combined filtering:** ```json {\"metadata\":[{\"path\":\"speaker\",\"operator\":\"Equal\",\"valueText\":\"John\"}],\"video_info\":[{\"path\":\"has_audio\",\"operator\":\"Equal\",\"valueText\":\"true\"}],\"file\":[{\"path\":\"filename\",\"operator\":\"Like\",\"valueText\":\"%.mp4\"}]} ```")] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[

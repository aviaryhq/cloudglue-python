...
class DefaultSegmentationConfig(BaseModel):
    """
    Configuration for video segmentation. **Choose a strategy and provide ONLY the corresponding config:**  • **uniform**: Provide `uniform_config`, do NOT provide other configs • **shot-detector**: Provide `shot_detector_config`, do NOT provide other configs • **narrative**: Provide `narrative_config`, do NOT provide other configs  Optionally specify `start_time_seconds` and `end_time_seconds` to limit segmentation to a portion of the video.
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["strategy", "uniform_config", "shot_detector_config", "narrative_config", "keyframe_config", "start_time_seconds", "end_time_seconds"]

    @field_validator('strategy')
    def strategy_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['uniform', 'shot-detector', 'narrative']):
            raise ValueError("must be one of enum values ('uniform', 'shot-detector', 'narrative')")
        return value
